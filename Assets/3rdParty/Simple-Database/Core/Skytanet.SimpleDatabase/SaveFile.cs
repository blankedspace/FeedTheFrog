...
namespace Skytanet.SimpleDatabase {
    ...
    public class SaveFile {
        ...
        public SaveFile(Stream treeFile, Stream blockFile)
        {
            this.OpenSaveFile(treeFile, blockFile);
        }
        /// <summary>
        ...
        private void OpenSaveFile(String filename, String path) {
            ...
            try {
                tree = BplusTreeBytes.Initialize(System.IO.Path.Combine(path, filename+".dat"), System.IO.Path.Combine(path, filename + ".block"), 255);
            } catch (System.IO.IOException) {
                try {
                    tree = BplusTreeBytes.ReOpen(System.IO.Path.Combine(path, filename + ".dat"), System.IO.Path.Combine(path, filename + ".block"));
        ...
        private void OpenSaveFile(Stream treeFile, Stream blockFile)
        {
            if (this.IsOpen())
            {
                Debug.LogError("You are tring to open an already opened save file (" + "\"" + filename + "\")");
            ...
            try
            {
                tree = BplusTreeBytes.ReOpen(treeFile, blockFile);
            }
            catch (System.IO.IOException)
            {
                try
        ...
        public static void DeleteSaveFile(string filename, string path) {
            try {
                File.Delete(System.IO.Path.Combine(path, filename + ".dat"));
